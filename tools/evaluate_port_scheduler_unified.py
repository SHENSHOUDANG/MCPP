...
def run_unified_rollout(env, seed: int, policy: str, model: HeterogeneousMappo | None = None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        ...
        trace.append(
            {
                ...
                "mean_assigned_scheduling_wait": float(info["metrics"].get("mean_assigned_scheduling_wait", 0.0)),
                "mean_all_scheduling_wait_truncated": float(info["metrics"].get("mean_all_scheduling_wait_truncated", 0.0)),
                "p95_scheduling_wait": float(info["metrics"].get("p95_scheduling_wait", 0.0)),
    ...
    summary = {
        ...
        "mean_assigned_scheduling_wait": float(final_info["metrics"].get("mean_assigned_scheduling_wait", 0.0)),
        "mean_all_scheduling_wait_truncated": float(final_info["metrics"].get("mean_all_scheduling_wait_truncated", 0.0)),
        "p95_scheduling_wait": float(final_info["metrics"].get("p95_scheduling_wait", 0.0)),
